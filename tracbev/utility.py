...
def save(data: gpd.GeoDataFrame, uc, col_select, uc_dict):
    ...
    save_path_csv = pathlib.Path(uc_dict["result_dir"], filename + '.csv')
    save_path_gpkg = pathlib.Path(uc_dict["result_dir"], filename + '.gpkg')
...
def parse_data(scenario):
    ...
    scenario_path = pathlib.Path('scenarios', scenario)
