...
def parse_data(args):
    ...
    scenario_path = pathlib.Path(".").resolve().parent / "scenarios" / args.scenario
    ...
    charge_info = pd.read_csv(
        pathlib.Path(data_dir, charge_info_file), sep=";", index_col="usecase"
    )
    ...
    if run_public:
        ...
        config_dict.update(
            {"poi_data": public_data, "public_positions": public_positions}
        )
    ...
    if run_home:
        ...
        config_dict.update(
            {
                "sfh_available": parser.getfloat(
                    "uc_params", "single_family_home_share"
                ),
                "sfh_avg_spots": parser.getfloat(
                    "uc_params", "single_family_home_spots"
                ),
                "mfh_available": parser.getfloat(
                    "uc_params", "multi_family_home_share"
                ),
                "mfh_avg_spots": parser.getfloat(
                    "uc_params", "multi_family_home_spots"
                ),
                "zensus": zensus_data,
            }
        )
    ...
    if run_work:
        ...
        work_dict = {
            "retail": work_retail,
            "commercial": work_commercial,
            "industrial": work_industrial,
        }
...
def parse_car_data(args):
    ...
    home_charging_prob = simbev_meta.loc["charging_probabilities", "config"][
        "private_charging_home"
    ]
...
def parse_potential_data(args):
    ...
    region_data = pd.read_csv(
        pathlib.Path(scenario_path, "regions.csv"), converters={"AGS": lambda x: str(x)}
    )
...
def run_use_cases(data_dict):
    ...
    if data_dict["run_public"]:
        uc.public(data_dict["public_positions"], data_dict["poi_data"], data_dict)
    ...
    if data_dict["run_home"]:
        if "home_prob" in data_dict.keys():
            ...
            uc.home(data_dict["zensus"], data_dict, 0, 0)

    if data_dict["run_work"]:
        uc.work(data_dict["work"], data_dict["work_dict"], data_dict)
...
def run_tracbev_default(data_dict):
    ...
    result_dir = pathlib.Path(
        "results", "{}_{}".format(data_dict["scenario_name"], timestamp)
    )
    ...
    for key, timeseries in data_dict["timeseries"].items():
        ...
        region = gpd.GeoSeries(
            region
        )  # format to geo series, otherwise problems plotting
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="TracBEV tool for allocation of charging infrastructure"
    )
    parser.add_argument(
        "scenario",
    ...
    parser.add_argument(
        ...
        default="default",
        type=str,
        help="Choose simulation mode: default "
